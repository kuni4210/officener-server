...
- danfojs로 엑셀처리를 진행했다. 그 이유는 내가 파이썬에서 데이터핸들링을 할때 사용하는 라이브러리인 pandas 와 비슷했기 때문에 danfojs를 선택했었다. 하지만 danfojs는 문제가 있었다. 첫번째는 tensorflow를 사용하기 때문에 서버를 실행할 때마다 현재 TensorFlow 바이너리가 시스템에서 사용 가능한 CPU 기능을 최적으로 활용하고 있다는 경고 메시지가 (
```
I tensorflow/core/platform/cpu_feature_guard.cc:151] This TensorFlow binary is optimized with oneAPI Deep Neural Network Library (oneDNN) to use the following CPU instructions in performance-critical operations:  AVX2 FMA. To enable them in other operations, rebuild TensorFlow with the appropriate compiler flags.
```
 발생했고, 라이브러리 자체가 용량이 무거웠다. 두번째는 csv-utf-8에 대해서는 인코딩을 처리해줄 수 없다는 것이었다.  이 두가지 이유로 xlsx로 라이브러리 교체 작업을 진행하였다. xlsx는 인코딩도 자동으로 처리를 하였고, 라이브러리 자체도 가벼워서 danfojs와는 다른 방식으로 구현을 했지만 두가지 원인을 모두 해결할 수 있었다.
