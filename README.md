...
- 기존의 웹은 ssg 방식으로 구현이 돼있어서 nginx와 연결을 할 때 build 파일의 위치를 찾아서 연결을 하는 구조였다. ssr 방식으로 바꾸고 그것을 연결할때 웹에 대한 지식이 없어 헤맸다. 나는 root   /home/ubuntu/빌드 파일의 경로, index  index.html index.htm; 이런식으로 해결이 될거라고 생각했기에 빌드파일의 위치를 계속 찾았지만 결국에는 구조가 아예 다르다는 사실을 알게되었다. 웹서버를 따로 실행시킨 후 그 포트번호에 맞춰 proxy_pass  http://localhost:4000; 이런시으로 해결할 수 있었다.
